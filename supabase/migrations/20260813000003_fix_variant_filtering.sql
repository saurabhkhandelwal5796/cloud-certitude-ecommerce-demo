-- Master migration to fix filtering-vs-projection architecture and restore dynamic facet logic.
-- This ensures that variants matching the user's dynamic attributes and price limits are always prioritized
-- and returned correctly in the UI. If no variant matches, the product is completely excluded.
...
BEGIN
  ...
  FROM public.products p
    INNER JOIN LATERAL (
      SELECT calc.variant_id, calc.price, calc.discount_percent, calc.images, calc.gst_rate
      FROM (
        SELECT v.id AS variant_id, v.price, 
               CASE WHEN v.price > 0 AND v.discounted_price IS NOT NULL AND v.discounted_price < v.price THEN ROUND(((v.price - v.discounted_price) / v.price) * 100) ELSE 0 END AS discount_percent, 
               v.images, v.gst_rate, v.is_primary, v.created_at
        FROM public.product_variants v
        WHERE v.product_id = p.id AND v.is_active = true
          AND (
            p_filters = '{}'::jsonb OR p_filters IS NULL
            OR NOT EXISTS (SELECT 1 FROM jsonb_object_keys(p_filters - 'Brand' - 'Category' - 'Rating' - 'Discount'))
            OR (
              SELECT COUNT(*)
      ...
      WHERE (p_price_max IS NULL OR (calc.price * (1.0 - COALESCE(calc.discount_percent, 0) / 100.0)) <= p_price_max)
        AND (
          p_filters = '{}'::jsonb OR p_filters IS NULL
          OR NOT (p_filters ? 'Discount')
          OR (
            COALESCE(calc.discount_percent, 0) >= (
              SELECT MIN(REPLACE(val, '% and above', '')::numeric)
              FROM jsonb_array_elements_text(p_filters->'Discount') AS val
            )
          )
        )
      ORDER BY calc.is_primary DESC, calc.created_at ASC
      LIMIT 1
    ) pv ON true
    ...
    INNER JOIN LATERAL (
      ...
      FROM (
        ...
        WHERE v.product_id = p.id AND v.is_active = true
          AND (
            p_filters = '{}'::jsonb OR p_filters IS NULL
            OR NOT EXISTS (SELECT 1 FROM jsonb_object_keys(p_filters - 'Brand'))
            OR (
              SELECT COUNT(*)
