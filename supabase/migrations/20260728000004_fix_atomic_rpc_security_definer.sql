-- ============================================================
-- Migration: Fix Atomic RPC Security Definer & Snapshot Quantity
-- Date: 2026-07-28
-- Description: Updates checkout_order_atomic and restore_inventory_atomic
-- with SECURITY DEFINER so they execute with elevated database permissions.
-- This prevents customer RLS policies on product_variants from blocking
-- SELECT ... FOR UPDATE row locks while keeping direct client write access locked down.
-- Also incorporates COALESCE quantity extraction for OrderItemSnapshot support.
-- ============================================================

-- ─── 1. Update checkout_order_atomic with SECURITY DEFINER ────────────────────
...
BEGIN
  ...
  LOOP
    ...
    -- Lock variant row for atomic read/write (bypasses RLS due to SECURITY DEFINER)
...
-- ─── 2. Update restore_inventory_atomic with SECURITY DEFINER ─────────────────
...
-- ─── 3. Reaffirm Execution Permissions (Security Audit C2) ─────────────────────
-- Ensure anonymous public cannot execute these transactional routines;
-- only authenticated users can trigger checkout and cancellations.
