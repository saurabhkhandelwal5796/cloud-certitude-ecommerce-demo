-- ============================================================
-- Migration: Phase 19 - Discount and Rating Facets
-- Date: 2026-07-31
--
-- 1. Updates get_node_facets to generate Rating and Discount facets
-- 2. Updates filter_products_by_node to filter by Rating and Discount
-- 3. Updates global_product_search to generate & filter by Rating and Discount
-- ============================================================
      ...
      OR (
        ...
        (
          ...
          OR EXISTS (
            ...
            WHERE (d_val = '70% or more' AND COALESCE(p.discount_percent, 0) >= 70)
               OR (d_val = '50% or more' AND COALESCE(p.discount_percent, 0) >= 50)
               OR (d_val = '25% or more' AND COALESCE(p.discount_percent, 0) >= 25)
               OR (d_val = '10% or more' AND COALESCE(p.discount_percent, 0) >= 10)
          )
        )
        AND
        (
